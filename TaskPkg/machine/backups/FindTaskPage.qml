...
Rectangle {
    ...
    function update_task_select_model() {
        ...
        if( gn_current_page < gn_total_page ) {
            for( var i = (gn_current_page-1)*gn_show_rows; i < gn_current_page*gn_show_rows; ++i ) {
                task_find_model.append( {"name":file_list.get_import_task_file_name( i ), "value_index": i%5, "select_btn_st":false} )
            }
            ...
            for( var j = (gn_current_page-1)*gn_show_rows; j < gn_total_count; ++j ) {
                task_find_model.append( {"name": file_list.get_import_task_file_name( j ), "value_index": j%5, "select_btn_st":false } )
            }
            if( gn_current_page == 1 ) {
                btn_pre.enabled = false
            } else {
            ...
            for(var n = 0; n < file_list.get_import_task_file_count(); ++n ){
                ...
                task_find_model.append( {"name": QStringfile_list.get_import_task_file_name( n ), "value_index": n%5, "select_btn_st":false } )
            }
            btn_pre.enabled = false
            btn_next.enabled = false
        }
        find_task_dlg.focus = true
        update_btn()
    }
    ...
    Rectangle {
        ...
        Rectangle {
            ...
            Text {
                ...
                text: qsTr("SD卡中发现作业包")
                color: "#7CA6C9"
                font.pixelSize: change_model.is_640x480_size() ? 55:45
            }
        }
        ...
        Component {
            ...
            Rectangle {
                ...
                Row {
                    ...
                    spacing: parent.width - row_name.width - 260
                    ...
                    Rectangle {
                        ...
                        MouseArea{
                            ...
                            onClicked: {
                                gn_current_index = n_index
                                delegate_cell.b_selected = !delegate_cell.b_selected
    ...
    function get_key_ok() {
        ...
        if(btn_return.focus) {
            ...
            if(gn_current_index >= 0 && gn_current_index < task_find_model.count) {
                gn_current_st = task_find_model.get(gn_current_index).select_btn_st
